...
def cut_license(afterimg,rect):
    ...
    # mask = np.astype('uint8')
    #创建背景模型  大小只能为13*5，行数只能为1，单通道浮点型
    bgdModel = np.zeros((1, 65), np.float64)
    #创建前景模型
    fgdModel = np.zeros((1, 65), np.float64)
    #分割图像
    cv.grabCut(afterimg, mask, rect_copy, bgdModel, fgdModel, 10, cv.GC_INIT_WITH_RECT)
    mask2 = np.where((mask == 1) | (mask == 0), 0, 1).astype('uint8')
    img_show = afterimg * mask2[:, :, np.newaxis]
    # 分割车牌与背景
    return img_show[rect_copy[1]:rect_copy[1] + rect_copy[3], rect_copy[0]:rect_copy[0] + rect_copy[2]]
